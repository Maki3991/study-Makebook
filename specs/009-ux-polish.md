...
6 级阶梯 —— **本轮重调，`globals.css` 数值需修改**（2026-07-25 决议：采用整改清单 P1-2 的数值）：

```
--text-display  4.5rem    72px   Hero
--text-monument 6rem      96px   仅清算价纪念碑（§6.1），不进常规阶梯
--text-h1       2.5rem    40px   页面标题
--text-h2       1.25rem   20px   区块标题，font-weight 600
--text-label    0.6875rem 11px   表头 / 状态 / 单位 / 区块编号，大写 + 字距 .14em
--text-body     0.9375rem 15px   正文
--text-micro    0.75rem   12px   脚注 / 时间戳，--ink-3
```

⚠️ 与旧值的差异（旧：display 48 / h1 36 / h2 24 / body 14 / small 12 / micro 10）：
`--text-small` **移除**，由 `--text-label`（11px，语义是标签而非小号正文）取代。
正文 14 → 15px，需同步复核行高与 section 间距。

迁移映射：

| 现状 | 次数 | 改为 | 性质 |
|---|---|---|---|
| `text-sm`（14px） | 95 | `text-body`（15px） | 正文整体上调 1px |
| `text-xs`（12px） | 49 | `text-micro`（12px） | 数值不变，只换名 |
| `text-base font-semibold`（区块标题） | ~10 | `text-h2`（20px/600） | 区块标题 16 → 20px |
| `text-base` 其余 | ~11 | `text-body` | 收敛到阶梯 |
| `text-micro`（旧 10px，均为大写标签用途） | 11 | `text-label`（11px） | 语义归位 |

区块标题与正文当前只差 2px，层级几乎不存在——这是项目页读起来像一张长清单的直接原因。
...
## 11. 合并「前端最终整改清单」（2026-07-25 追加）

外部整改清单与本 spec 合并后的裁决。**该清单强于版面、盲于链路**——它未覆盖任何交互与故障项（§5 修血全部不在其中），两者互补，均不可丢。

### 11.1 已被本 spec 覆盖（不重复列）

| 清单项 | 对应 |
|---|---|
| P0-1 演示批次截止时间 | §7（bracelet +7 天重部署，覆盖 Expo 全程） |
| P0-3 表格与曲线数据矛盾 | §4（根因已定位：`quote-table.tsx:43` 用出厂价，曲线用零售价） |
| P0-4 表格缺零售价列 | §4.3 |
| P0-5 移动端导航 | §5-1（清单的汉堡菜单方案更具体，采纳） |
| P1-2 六档字号阶梯 | §1.4（数值以清单为准，已更新） |
| P2-6 价格拆解进下单框 | §3.2 C3 |
| P2-6 发起方标识 | §3.2 C1 |
...
| # | 项 | 说明 |
|---|---|---|
| N-1 | **粘性导航穿透** | 已核实：`top-bar.tsx:11` 为 `bg-canvas/95`（95% 不透明）。改为 `bg-canvas` 全不透明；复核 `z-index` 与 `scroll-padding-top` |
| N-2 | **清算价数字纪念碑** | 本轮单项收益最高。Campaign 页顶部独立展示区：Label 档标题 + `--text-monument` mono 数字 + 单位基线对齐 + 一行 micro 上下文。抽象为可复用组件，套用到「ORDERS 5 / 50」等关键指标 |
| N-3 | 产品图未显示（移动端首页底部灰块） | 先定位根因（路径 / 加载失败 / 容器高度 0），不得用 CSS 掩盖 |
| N-4 | 移动端左右边距不对称 | 容器 padding 左右等距 20–24px，逐页检查 |
| N-5 | 移动端 Hero 死空白 | 副标题到 CTA 之间空出近一屏，主 CTA 被推出首屏。移动端断点收紧 |
| N-6 | TESTNET 横幅链接折行 | `Get test INJ →` 整体 nowrap；空间不足时横幅改两行 |
| N-7 | mono 扩展 | 倒计时、批次编号、表格数值列、SPECS 规格值统一 mono + tabular-nums |
| N-8 | 倒计时省略零单位 | `0d 17h 26m` → `17h 26m` |
| N-9 | 状态标识统一 | 现有四种写法（描边大写 / 绿色句首 / 琥珀长句 / 描边大写）统一为：全大写 + 字距 .12em + 1px 描边 + 语义色；长解释降级为 micro 另起一行 |
| N-10 | Console 双 Connect wallet | 同屏两个按钮，删除卡片内那个 |
| N-11 | 语言切换含义不明 | `EN / 中` 改为同时显示两项并高亮当前；移动端收进汉堡菜单 |
| N-12 | 收敛用色 | 可行状态改 `--ink-1` + 实心方点，不可行 `--ink-3`；橘红只留给主 CTA（每屏至多一个）与中标档位高亮 |
| N-13 | 产品图背景冷灰 vs 暖底 | 棚拍背景偏冷，贴在 `#F5F3EF` 上像补丁。**可用脚本合成到 `--paper-0` 并保留投影**，无需等设计侧 |
| N-14 | 品牌标记（移动端） | 已装 `public/brand/mark-{512,128,64}.png`（透明抠图版；旧 `makebook-monogram.png` 为白底，在暖纸底上会露白斑）。<768px 顶栏只用 mark，`flex-shrink:0` + 固定宽高比 |

### 11.3 🔴 拦截：不可执行

**P2-6「SPECS 补充至 6–8 项」——禁止对 FRAME-01 执行。**

SPECS 来源为 `public/manifests/frame-01.json`，其 canonical hash `0x92e96e07…cc6ec` 已冻结在三套 Campaign 的 constructor 参数中。
修改 manifest → hash 变更 → 页面上「产品说明书 hash 已校验，与链上一致 ✓」翻转为「说明书 hash 与链上不一致，请暂缓下单」，"AI 编译 → 人工确认 → 上链锚定"的可信链条当场崩塌。
AGENTS.md 红线亦明确禁止改动 `public/manifests/`。

**唯一安全路径**：仅为 §7 中重新部署的 bracelet 批次提供规格更丰富的 manifest（新 hash 随新合约一同上链）。FRAME-01 A/B 保持不动。

### 11.4 降级（排在最后，砍掉不影响验收）

| 项 | 理由 |
|---|---|
| P1-3 区块编号 `SECTION 03 —` | 纯装饰，1h+ |
| P2-3 需求曲线工程图纸化（8 条子项） | 实测 3h+，而曲线是当前全站完成度最高的部分，边际收益最低 |
| P2-4 全站基准线对齐 | 中收益中成本 |
| P2-5 清算揭晓动效 | 明早现场 settle，动效出错即毁掉高光时刻。若做，只做数字滚动一段，且必须支持 `prefers-reduced-motion` |

### 11.5 文案权限裁决

清单「明确不做」列有「修改 PRD 第 15 章已定义的文案原文」，与 §2 冲突。
**裁决（2026-07-25，产品负责人）**：按 §2 打磨全站文案。
**唯一例外**：`back-drawer` 签名前的两段知情同意原文（不可撤单 / 信息公开）与双勾选逐字不动——那不是文案，是对用户的保护。
